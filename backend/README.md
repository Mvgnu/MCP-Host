...
The Rust scaffolding lives in `backend/src/keys/` and exposes a `ProviderKeyService` (`key: provider-keys-service`) alongside contract types for API/CLI parity. Registration now requires attestation digests (base64 encoded SHA-256) plus matching attestation signatures, recording a verification timestamp before activating the key. Rotation requests can be submitted via `POST /api/providers/:provider_id/keys/:key_id/rotations`, which captures the attestation digest/signature pair, enforces a non-empty operator reference, flips the key into the `rotating` posture, and emits a `provider-keys-audit-event` entry tagged `rotation_requested`. Binding management is now live via `POST /api/providers/:provider_id/keys/:key_id/bindings` and `GET /api/providers/:provider_id/keys/:key_id/bindings`, which validate binding type/target pairs, deduplicate active attachments, persist durable records, and emit `binding_attached` audit events (fan-out through `pg_notify` when configured). Listings enforce provider ownership before returning binding payloads so CLI/console surfaces can reconcile workload coverage. REST handlers in `backend/src/keys_api.rs` validate digest/signature encodings, surface missing keys as `404`, reject inactive keys with `409`, translate duplicate bindings into `409`, and otherwise fall back to the previous `501 Not Implemented` status while runtime enforcement is being stitched together. Downstream surfaces should rely on the machine-readable `key:` annotations when binding to the contract.

Rotation SLA enforcement now lives in `ProviderKeyService::enforce_rotation_slas`, which queries upcoming/overdue deadlines, deduplicates audit events across cron invocations, and returns a structured report for observability jobs. Emergency revocation is exposed through `ProviderKeyService::revoke_key` and the `/api/providers/:provider_id/keys/:key_id/revocations` route, forcing runtime policy re-evaluation while emitting paired `revocation_initiated`/`revocation_completed` audit entries. These mutations flow through the same `pg_notify` channel so lifecycle SSE listeners receive posture deltas immediately.

A new `backend/src/audit.rs` module provides `ProviderKeyAuditFilter` and `query_provider_key_events` helpers that power REST/CLI audit queries. Handlers marshal the scoped logs via `/api/providers/:provider_id/keys/audit`, while the CLI `mcpctl keys audit` command formats entries into tables or JSON for forensic workflows.
